...
{ content.map((a,i)=>{
  ...
  return(
  <div key={i} className={` flex  items-center justify-start w-full h-[680px] bg-transparent overflow-visible ${isReverse?'flex-row-reverse':'flex-row'}`}>
  <div className="relative flex flex-col items-center justify-center w-1/2 h-full p-8 bg-transparent gap-7 ">
    <div className="relative flex flex-col items-start justify-start w-full gap-6 bg-transparent">
      <div className="relative flex flex-col items-start justify-start w-full bg-transparent gap-7">
        <h3 className="text-[51px] font-['PoorRichard'] text-left text-[#034401] m-0 p-0">{heading}</h3>
  ...
  <Image src={image} alt='' height={1000} width={1000} className="inset-0 object-cover w-1/2 h-full rounded-lg"/>
</div>)
})    

      ...
      <div className="relative w-full max-w-[350px] flex flex-col gap-4 p-4 bg-[#FAFAFA] rounded-lg cursor-pointer  hover:bg-[#F3FFE5] hover:shadow-md pb-4">
        ...
        <div className="flex gap-2">
          {Array.from({ length: 1 }).map((_, i) => (
            <div
              key={i}
              className="flex items-center justify-center px-3 py-1 bg-white group-hover:bg-[#F3FFE5] rounded-full ring-1 ring-[#F5F5F5] transition-all duration-300 gap-x-2 group-hover:ring-[#D2ECB4]"
            >
              <p className="text-sm ">Treatment for:</p>
              <p className=" font-poppins">{forTreatmentOf || " _ "}</p>
            </div>
          ))}
        </div>
      </div>
